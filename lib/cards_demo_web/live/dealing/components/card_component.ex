defmodule CardsDemoWeb.Dealing.CardComponent do
  ...
  @impl true
  def mount( socket) do
    { :ok, socket, temporary_assigns: [ entries: [], dom_map: nil]}
  end

  # Adds the list of entry assigns for entries as temporary assigns.
  @impl true
  def update( new_assigns, socket) do
    { entries, card_update} = Listiller.apply( CardsDemoWeb.Dealing.EntryComponent, socket.assigns, new_assigns)
    { :ok, socket
           |> assign( new_assigns)
           |> assign( :entries, entries)
           |> assign( :card_update, card_update)}
  end

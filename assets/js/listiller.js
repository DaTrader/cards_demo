/**
 * Listill Library JS Interoperability
 */
...
export function prepForSorting( listill, el, getsId) {
  const dstBareId = el.dataset[ listill.sortAttr];
  if( !dstBareId) return null;
...
export function applyCall( elId, fun) {
  elementById( elId).dispatchEvent( new CustomEvent( "applyCall", { detail: { fun: fun}}));
}
